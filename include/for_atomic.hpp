///
/// @file  for_atomic.hpp
///        The for_atomic_add() macro is a workaround for a severe
...
/// AC(x) with x >= 1e22.
///
/// for_atomic_add(var, condition, inc)
/// Is the same as:
///
/// #pragma omp for nowait schedule(dynamic)
/// for (auto var = atomic_var; condition; var += inc)
///
#define for_atomic_add(var, condition, inc) \
  for (auto var = atomic_##var.fetch_add(inc, std::memory_order_relaxed); \
       condition; var = atomic_##var.fetch_add(inc, std::memory_order_relaxed))
