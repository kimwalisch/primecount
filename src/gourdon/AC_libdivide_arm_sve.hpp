///
/// @file  AC_libdivide_arm_sve.hpp
/// @brief ARM SVE libdivide implementation of the A + C
...
#ifndef AC_LIBDIVIDE_ARM_SVE_HPP
#define AC_LIBDIVIDE_ARM_SVE_HPP
...
#endif
ALWAYS_INLINE svuint64_t divide_libdivide_arm_sve(svbool_t pg,
                                                  svuint64_t xp,
                                                  const uint64_t* magic,
                                                  const uint8_t* shift)
{
  svuint64_t m = svld1_u64(pg, magic);
  svuint64_t shifts = svld1ub_u64(pg, shift);
  svuint64_t q = svmulh_u64_x(pg, xp, m);
  svuint64_t t = svsub_u64_x(pg, xp, q);
  t = svadd_u64_x(pg, svlsr_n_u64_x(pg, t, 1), q);
  return svlsr_u64_x(pg, t, shifts);
}
...
template <typename T, int MULTIPLIER>
  ...
  __attribute__ ((target ("+sve")))
#endif
ALWAYS_INLINE T sum_pi_libdivide_arm_sve(uint64_t xp,
                                         uint64_t i,
                                         uint64_t last,
                                         uint64_t b,
                      ...
                      const SegmentedPiTable& segmentedPi)
{
  ...
  sum += sum_pi_libdivide_arm_sve<T, 1>(xp, i, max_i1, 2, dividers, segmentedPi);
  i = max(i, max_i1 + 1);

  // x / pq < y && low <= x / pq < high
  sum += sum_pi_libdivide_arm_sve<T, 2>(xp, i, max_i2, 2, dividers, segmentedPi);
                       ...
                       const SegmentedPiTable& segmentedPi)
{
  ...
  if (min_m < max_prime)
  {
    ...
    sum -= sum_pi_libdivide_arm_sve<T, 1>(xp, min_i, max_i, b, dividers, segmentedPi);
  }
  ...
  if (min_q < max_q)
  {
    ...
    for (uint64_t i = min_q_i; i <= max_q_i; i++)
    {
      ...
      sum += sum_pi_libdivide_arm_sve<T, 1>(xpq, min_j, max_j, b, dividers, segmentedPi);
    }
  }
                       ...
                       const SegmentedPiTable& segmentedPi)
{
  ...
  sum += sum_pi_libdivide_arm_sve<T, 1>(xp, i, pi_conj_lo, b, dividers, segmentedPi);
  i = pi_conj_lo + 1;

  // Reflected leaves: counted once as a sparse leaf, once as a conjugate.
  sum += sum_pi_libdivide_arm_sve<T, 2>(xp, i, pi_conj_hi, b, dividers, segmentedPi);
  i = pi_conj_hi + 1;

  // Sparse leaves above the reflected range
  sum += sum_pi_libdivide_arm_sve<T, 1>(xp, i, pi_min_clustered, b, dividers, segmentedPi);
...
template <typename T,
  ...
  __attribute__ ((target ("+sve")))
#endif
T AC_OpenMP_libdivide_arm_sve(T x,
                              int64_t y,
                              int64_t z,
                              int64_t k,
                              int64_t x_star,
    ...
    while (loadBalancer.get_work(thread))
    {
      ...
      for (; low < limit; low += segment_size)
      {
        ...
        if (low < z)
        {
          ...
          for (int64_t b = min_c1; b <= pi_sqrtz; b++)
          {
            ...
            if (xp <= pstd::numeric_limits<uint64_t>::max())
              sum -= C1_libdivide_arm_sve(xlow, xhigh, uint64_t(xp), b, y, z, dividers, primes, pi, segmentedPi);
            else
        ...
        for (int64_t b = min_c2; b <= max_c2_clustered; b++)
        {
          ...
          if (xp <= pstd::numeric_limits<uint64_t>::max())
            sum += C2_libdivide_arm_sve(xlow, xhigh, uint64_t(xp), y, b, pi_y, max_clustered_global, prime, dividers, pi, segmentedPi);
          else
        ...
        for (int64_t b = min_a; b <= max_a; b++)
        {
          ...
          if (xp <= pstd::numeric_limits<uint64_t>::max())
            sum += A_libdivide_arm_sve(xlow, xhigh, uint64_t(xp), y, prime, dividers, pi, segmentedPi);
          else
