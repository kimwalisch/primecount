                      ...
                      const SegmentedPiTable& segmentedPi)
{
  ...
  sum += sum_pi_libdivide_arm_sve<T, 1>(xp, i, max_i1, 2, lprimes, segmentedPi);
  i = max(i, max_i1 + 1);

  // x / pq < y && low <= x / pq < high
  sum += sum_pi_libdivide_arm_sve<T, 2>(xp, i, max_i2, 2, lprimes, segmentedPi);
                       ...
                       const SegmentedPiTable& segmentedPi)
{
  ...
  if (min_m < max_prime)
  {
    ...
    sum -= sum_pi_libdivide_arm_sve<T, 1>(xp, min_i, max_i, b, lprimes, segmentedPi);
  }
  ...
  if (min_q < max_q)
  {
    ...
    for (uint64_t i = min_q_i; i <= max_q_i; i++)
    {
      ...
      sum += sum_pi_libdivide_arm_sve<T, 1>(xpq, min_j, max_j, b, lprimes, segmentedPi);
    }
  }
                       ...
                       const SegmentedPiTable& segmentedPi)
{
  ...
  sum += sum_pi_libdivide_arm_sve<T, 1>(xp, i, pi_conj_lo, b, lprimes, segmentedPi);
  i = pi_conj_lo + 1;

  // Reflected leaves: counted once as a sparse leaf, once as a conjugate.
  sum += sum_pi_libdivide_arm_sve<T, 2>(xp, i, pi_conj_hi, b, lprimes, segmentedPi);
  i = pi_conj_hi + 1;

  // Sparse leaves above the reflected range
  sum += sum_pi_libdivide_arm_sve<T, 1>(xp, i, pi_min_clustered, b, lprimes, segmentedPi);
    ...
    while (loadBalancer.get_work(thread))
    {
      ...
      for (; low < limit; low += segment_size)
      {
        ...
        if (low < z)
        {
          ...
          for (int64_t b = min_c1; b <= pi_sqrtz; b++)
          {
            ...
            if (xp <= pstd::numeric_limits<uint64_t>::max())
              sum -= C1_libdivide_arm_sve(xlow, xhigh, uint64_t(xp), b, y, z, lprimes, primes, pi, segmentedPi);
            else
        ...
        for (int64_t b = min_c2; b <= max_c2_clustered; b++)
        {
          ...
          if (xp <= pstd::numeric_limits<uint64_t>::max())
            sum += C2_libdivide_arm_sve(xlow, xhigh, uint64_t(xp), y, b, pi_y, max_clustered_global, prime, lprimes, pi, segmentedPi);
          else
        ...
        for (int64_t b = min_a; b <= max_a; b++)
        {
          ...
          if (xp <= pstd::numeric_limits<uint64_t>::max())
            sum += A_libdivide_arm_sve(xlow, xhigh, uint64_t(xp), y, prime, lprimes, pi, segmentedPi);
          else
