...
SegmentedPiTable::SegmentedPiTable(uint64_t low,
  ...
  : counts_(threads),
    ...
    threads_(threads)
{
  // For primecount-backup the minimum segment size should be
  // very large (8 MiB) in order to reduce thread scheduling overhead.
  uint64_t numbers_per_byte = 240 / sizeof(pi_t);
  uint64_t min_segment_size = (8 << 20) * numbers_per_byte;
