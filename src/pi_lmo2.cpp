...
///        counting algorithm. This implementation does not use a
///        special data structure in S2(x) which counts the number of
///        unsieved elements in O(log N) operations.
///
           ...
           std::vector<int32_t>& mu)
{
  ...
  // next multiples to cross-off
  std::vector<int64_t> next(primes.size());
  ...
  for (int64_t low = 1; low < limit; low += segment_size)
  {
    ...
    {
      int64_t k = next[b];
      for (int64_t p = primes[b]; k < high; k += p)
        sieve[k - low] = 0;
      next[b] = k;
    }
    ...
    for (int64_t b = c; b < a; b++)
    {
      ...
      int64_t k = next[b + 1];
      for (int64_t p2 = primes[b + 1] * 2; k < high; k += p2)
        sieve[k - low] = 0;
      next[b + 1] = k;
    }
  }
