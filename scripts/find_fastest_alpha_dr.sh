...
# Usage:
#   ./find_fastest_alpha_dr.sh [--start=n] [--stop=n] [-t=threads]
# Description:
...
do
    case $i in
        ...
        *)
        ...
        echo "Usage:"
        echo "  ./find_fastest_alpha_dr.sh [--start=n] [--stop=n] [-t=threads]"
        echo "Description:"
...
{
    sleep 0.2
    seconds=$(./primecount $1 -d --time | grep Seconds | cut -d':' -f2 | cut -d' ' -f2)
    echo $seconds
}
